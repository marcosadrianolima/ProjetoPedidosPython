...
async def post_comanda(corpo: Comanda):
    try:
        ...
        # antes de abrir a comanda validar se ela já não está aberta, status == 0
        dados = session.query(ComandaDB) \
            .filter(ComandaDB.comanda == corpo.comanda) \
            .filter(ComandaDB.status == 0) \
            .all()
...
async def post_comanda_item(corpo: ComandaProdutos):
    try:
        ...
        session.add(dados)
        # session.flush()  # Descomentado se for necessário para garantir o persistente imediato
        session.commit()
...
# Edita item na Comanda - se quantidade for zero, exclui o item
...
async def put_comanda_item(corpo: ComandaProdutos):
    try:
        ...
        # se nova quantidade for zerada, exclui
        if corpo.quantidade == 0:
            session.delete(dados)
        else:
            # edita os dados do item
            dados.funcionario_id = corpo.funcionario_id
...
async def get_comanda_total(comanda_id: int):
    try:
        ...
        dados = session.query(
            func.sum(ComandaProdutoDB.quantidade * ComandaProdutoDB.valor_unitario).label("Total:")
        ).filter(ComandaProdutoDB.comanda_id == comanda_id).scalar()
